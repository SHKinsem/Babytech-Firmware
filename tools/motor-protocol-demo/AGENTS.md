...
- 用户在 2026-09-23 确认实机网页以任务路径和可用编辑空间为优先。指令实验室桌面保留连接、使能、故障和最近应答的紧凑摘要，详细反馈由明确按钮按需展开，展开时不重新压窄编辑区；移动端先选指令，选中后直接定位编辑区，并在编辑标题处提供更换指令入口。长参数表单保持发送区可达，固定停止按钮不得遮挡最后一项输入。参数说明必须可点按、可键盘关闭。编排队列桌面默认优先展开程序编辑，预览和板端进度与设置按需展开；板端状态仍在编辑区摘要显示。

- 用户在 2026-09-23 指出协议控制台顶部双行横幅浪费空间：桌面将品牌、CAN ID、连接状态和全局控制合并到约 60px 的单行工具栏，标签栏约 42px；窄屏可隐藏重复的协议与速率文案，但必须保留 CAN ID、连接状态和全部停止。不要让空白标题区挤占工作区。

- 用户在 2026-09-23 更新要求：下板实机网页的整体操作路径必须清晰；桌面、平板和手机的各主要窗口都可独立折叠，折叠后把空间让给仍在使用的窗口。窄屏工作模式通过完整菜单直达，常用操作容易到达，全部停止保持可见。收起窗口只隐藏显示，不卸载内容或清掉草稿、轮询和在途结果；折叠状态在会话中保留。验收覆盖 390×844、900×800、1280×800 和 1513×1039。
...
- 用户要求切换页面后保留刚编辑的参数。实机指令表单与 HEX 按电机 ID、指令、变体分别保留，常规试动按电机 ID 保留；同源浏览器刷新后恢复本机草稿。
- 草稿不是设备读回或已应用配置。恢复、切换和轮询不得自动发送指令，不持久化密码、使能、在线、应答或执行状态；已确认板端限制仍参与发送校验。
- 保留空字符串和未完成的输入，不因切换或限制刷新静默裁剪。原始 HEX 的未发送修改只能由明确的重置操作清除。
- 2026-09-23 用户要求参数表单直接填写常用单位，单位字样保持次要字号。X 固件的 0.1 RPM 和 0.1° 字段在界面分别以 RPM 和 ° 输入，允许一位小数并精确乘 10 入帧；协议编码、原始 HEX 和本机草稿仍使用原始整数，未完成或超精度输入保留原文并阻止发送。

## 调试日志与手动清除状态（2026-09-22）

- 用户要求可查看、导出的调试日志，保留操作、拒绝原因、超时、状态变化与观察到的 CAN 帧。日志应区分已提交、已入队、驱动器应答和机械完成；不得把轮询塞满事件列表，或在缺口处补造记录。
- 用户明确选择一个直接的“清除板端状态”按钮，不要做自动重新使能或恢复旧动作的流程。此按钮取消队列与 UART 待执行事务、尝试停止、清除易失的忙/故障/等待状态；保留配置、草稿和日志。
- 清除软件状态不是电机已经停稳的证据。停止发送失败和请求结果未知必须分别报告，不能自动重试；后续动作仍由用户显式操作。

## 最新覆盖决定：编排直接发送（2026-09-22）

- 用户否定此前编排队列的过度监督，要求类似 G-code 的直接执行器，执行边界由用户判断。本节覆盖上面所有队列必须等待 ACK、到位、静止或配置读回的旧要求；常规试动/指令实验室的独立实现不由此自动改写。
- 编排按顺序发送 CAN，只在用户写出的等待/持续时间处等待。未使能、无反馈、历史故障、未停稳或未到位不得拦截队列；不自动补使能或在失败/发送结束时追加停止。
- 只保留报文编码、单位换算和资源容量必需的检查，不套用手动试验策略。显式停止/取消/清状态照常执行。结束文案是“发送结束”，不能声称机械到位。
- 交付收敛：完成直发队列、清状态按钮和已有日志后停止扩展，仅执行必要回归和最终编译。

## 设备板不校验模式（2026-09-25）

- 用户要求设备板提供可持久化的全局开关，重启后保持开启状态；其范围包括设备板网页、直接 UART 指令和内置 Demo。开启时只检查语法与协议字段能否无损编码，发送指令不等待 ACK、位置、速度或到位证据，也不套用手动策略限值。
- 页面须显示当前模式，并把直通结果表述为已发送或脚本已发完，不宣称物理到位。故障、使能和停止的旧状态不得在此模式继续阻止发送；常规模式的现有行为保持。
- 开启时运行中的队列或 Demo 可继续发送并跳过反馈等待；运行中关闭返回 `control_mode_busy`。显式停止或失能终止尚未发送的队列步骤，不能暗中补发第二条电机指令。
