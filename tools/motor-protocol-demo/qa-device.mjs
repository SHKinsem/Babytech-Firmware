...
try {
 ...
 await page.getByText('使能已确认',{exact:true}).waitFor();
  ...
  await page.getByLabel('搜索指令名称或功能码').fill('速度');
  await page.locator('.library__item').filter({hasText:'速度'}).first().click();
  const rpm=page.getByLabel('速度（RPM）',{exact:true});
  await rpm.fill('60.');
  assert.equal(await rpm.inputValue(),'60.','unfinished scaled input stays visible as a draft');
  assert.equal(await send.isDisabled(),true,'unfinished scaled input must block sending');
  const beforeInvalidSend=posts.length;
  await rpm.fill('60');
  assert.equal(await send.isDisabled(),false,'valid whole RPM input should enable sending');
  await send.click();
  await page.waitForTimeout(100);
  assert.equal(posts.length,beforeInvalidSend+1);
  assert.deepEqual(posts.at(-1),{path:'/api/command',params:{hex:'01F600003C0258006B'}});
  checks.push('FieldRow 保留 60. 草稿并禁发，改为 60 RPM 后发送 600 个协议单位');
 ...
 assert.equal(await page.getByRole('button',{name:'忘记已保存网络'}).isDisabled(),true);checks.push('忘记网络与状态更新');
  ...
  const scaleDiagnostics=page.locator('.compact-panel--scale-diagnostics');
  const diagnosticsToggle=scaleDiagnostics.getByRole('button',{name:/诊断与 IO 设置/});
  if(await diagnosticsToggle.getAttribute('aria-expanded')!=='true')await diagnosticsToggle.click();
  assert.equal(await diagnosticsToggle.getAttribute('aria-expanded'),'true','IO settings must be reachable by opening the diagnostics panel');
