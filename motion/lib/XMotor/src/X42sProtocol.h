...
public:
    ...
    // Controller validates the logical command before entering this gateway.
    bool sendValidatedCommand(const uint8_t* bytes, uint8_t length) {
        return sendCommand(bytes, length);
    }
    using TraceSink = void (*)(void*, const CanRawFrame&, bool);
    void setTraceSink(TraceSink sink, void* context) { traceSink_ = sink; traceContext_ = context; }
