...
/**
  * @brief    设置快速梯形曲线位置模式的运动参数（X42S/Y42）
  * @param    addr  ：电机地址
  * @param    acc   ：加速加速度(RPM/s)	，0为CW，其余值为CCW
  * @param    dec   ：减速加速度(RPM/s)	，0为CW，其余值为CCW
  * @param    vel		：最大速度(RPM)			，范围0.0 - 3000.0RPM
  * @param    raf   ：相位/绝对运动标志	，0为相对上一次输入目标位置，1为绝对位置，2为相对电机当前实时位置
  * @param    snF   ：多机同步标志				，false为不启用，true为启用
	* @param    maxCur：最大电流(mA)				，范围0 - 6000mA
  * @retval   地址 + 功能码 + 命令状态 + 校验字节
  */
void X_V2_Set_QTrajPos_Params(uint8_t addr, uint16_t acc, uint16_t dec, float vel, uint8_t raf, bool snF, uint16_t maxCur)
{
  ...
  cmd[0]  =  addr;                      // 地址
  cmd[1]  =  0xF1;                      // 功能码
  cmd[2]  =  (uint8_t)(acc >> 8);       // 加速加速度(RPM/s)
  cmd[3]  =  (uint8_t)(acc >> 0);
  cmd[4]  =  (uint8_t)(dec >> 8);       // 减速加速度(RPM/s)
  cmd[5]  =  (uint8_t)(dec >> 0);
  cmd[6]  =  (uint8_t)(v >> 8);       	// 最大速度(RPM)
  cmd[7]  =  (uint8_t)(v >> 0);
  cmd[8]  =  raf;                       // 相位/绝对运动标志
  cmd[9]  =  snF;                       // 多机同步运动标志
	cmd[10] =  (uint8_t)(maxCur >> 8);    // 最大电流(mA)
...
/**
  * @brief    快速梯形曲线位置模式控制（X42S/Y42）
  * @param    addr  ：电机地址
  * @param    pos		：位置角度(带符号)，单位：0.1°
  * @retval   地址 + 功能码 + 命令状态 + 校验字节
  */
