...
本文保留 v1 历史协议。当前主控板与设备控制板已改用 [v2 四指令协议](protocol-v2.md)，两板须一起更新。以下字段不再是当前运行接口。
...
| Type | 名称 | 方向 | Payload |
| --- | --- | --- | --- |
| 1 | GetStatus | main-controller → device-controller | 空 |
| 2 | Status | device-controller → main-controller | 6 bytes，见下表 |

Status 沿用请求的 Sequence。main-controller 只接受当前待处理请求的响应。
自动查询与网页手动查询共享一个在途请求；每 500 ms 可重新查询。
设备控制板 1500 ms 没有有效响应时，main-controller 将其显示为离线。

| Status payload 偏移 | 内容 |
| --- | --- |
| 0–3 | 设备控制板 uptime，uint32 毫秒 |
| 4 | 状态：`0 = not_configured` |
| 5 | 能力位：bit 0 电机已接入、bit 1 传感器已接入；本阶段均为 0 |

设备控制板 uptime 是设备控制板本地时钟，不与主控板时钟比较。它不表示物理电机是否停止。

## 主控板网页接口

- `GET /`：固件内嵌页面。
- `GET /api/status`：读取主控板缓存的链路状态，不阻塞等待 UART。
- `POST /api/query`：触发或合并一次设备控制板查询；HTTP 202 仅表示请求已排入发送路径。
