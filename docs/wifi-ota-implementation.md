...
3. 用 [`package-ota.py`](../tools/package-ota.py) 分别生成发布目录。例如本机 PlatformIO 输出：
   ...
   ```powershell
   python tools/package-ota.py --board main-controller --build-dir main-controller/.pio/build/main-controller
   python tools/package-ota.py --board device-controller --build-dir device-controller/.pio/build/device-controller
   ```
