...
PR #15 最初提交完成了 P1 的软件基线、`MotorBus` 发送薄封装，以及 HTTP、Brain UART、DISPLAY Demo 到 `DeviceAPI` 的入口迁移。随后将全板唯一的自动查询调度器和派发时机迁入 `MotorBus`，各生产调用方通过窄接口登记/释放需求。它仍未迁走 `MotorControl` 的运动/回零状态机、`CommandQueue` 的程序运行状态或 `SyncRuntime` 的同步状态机；`DeviceAPI` 的回执和快照主要映射旧执行器的结果。因此这批工作是**入口迁移与 P2 查询所有权拆分**，不能称为整个控制逻辑重构完成。PR 保持草稿，后续核心拆分与验收另行记录。

| 目标阶段 | 当前实现事实 | 尚需完成 |
| --- | --- | --- |
| P1 行为基线 | 主机回归及两种 UART 配置编译通过；五轴 Demo/页面与 `await` 组合测试已加入。 | 本提交镜像的实机 CAN/机械验收。 |
| P2 总线与观测 | `MotorBus` 已持有唯一 `X42sProtocol` 与 `CanQueryScheduler`，负责自动查询预算、派发和 TX 后让行；`CommandQueue`、Demo、页面/配置入口改用窄接口。 | `SyncRuntime` 仍通过兼容出口直接持有可变调度器；显式原始读取不受自动查询配额约束。电机注册/观测所有权、完整流量仲裁及实机测量仍需完成。 |
| P3–P4 指令与程序运行 | 现有 `MotorControl`、`CommandQueue`、`SyncRuntime` 继续执行旧逻辑。 | 指令实例/结果句柄、独立运行器与原有行为的迁移验证。 |
