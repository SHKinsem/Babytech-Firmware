...
PR #15 最初提交完成了 P1 的软件基线、`MotorBus` 发送薄封装，以及 HTTP、主控板 UART、显示板 Demo 到 `DeviceAPI` 的入口迁移。随后将全板唯一的自动查询调度器和派发时机迁入 `MotorBus`，各生产调用方通过窄接口登记/释放需求。手动 `4C` 配置写入的 ACK、`0x22` 三包读回、校验、超时及取消状态已迁入独立的 `ConfigTransaction`；它只报告查询需求和结果，实际发送仍走 `MotorBus` 的共享预算。新增的配置事务契约测试和线级回归覆盖乱序、重复、陈旧回复、取消和失败；手动 Move/Home 的公共入口测试补充了互斥和停止证据。它仍未迁走 `MotorControl` 的运动/回零状态机、`CommandQueue` 的程序运行状态或 `SyncRuntime` 的同步状态机；`DeviceAPI` 的回执和快照主要映射旧执行器的结果。因此这批工作是**入口迁移与部分核心状态所有权拆分**，不能称为整个控制逻辑重构完成。PR 保持草稿，后续核心拆分与验收另行记录。
...
| P3–P4 指令与程序运行 | 新增 Move/Home/停止的公共边界迁移测试；现有 `MotorControl`、`CommandQueue`、`SyncRuntime` 仍执行旧运动和程序逻辑。 | 手动 Move/Home 单一状态所有者、指令实例/结果句柄、独立运行器与原有行为的迁移验证。 |
